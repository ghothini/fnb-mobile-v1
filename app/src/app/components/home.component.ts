...
export class homeComponent {
  ...
  routeIfLoggedIn(item: any = undefined, ...others) {
    ...
    try {
      ...
      bh.input = { item };
      bh.local = {};
      bh = this.sd_9VdaBCcQfHd0Yuzm(bh);
      //appendnew_next_routeIfLoggedIn
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_woetTvqXZ3w5YcgC');
    }
  }
  ...
  sd_23OWtAZAhXWQMroe(bh) {
    try {
      localStorage.setItem('selectedPage', JSON.stringify('Forgot Password'));
  ...
  async sd_9VdaBCcQfHd0Yuzm(bh) {
    try {
      if (
        ...
        bh = this.sd_7h1N1DtKW2Z1nvBf(bh);
      } else {
        bh = await this.sd_VVCZsbA3shYu72Xs(bh);
      }

      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_9VdaBCcQfHd0Yuzm');
    }
  }

  async sd_7h1N1DtKW2Z1nvBf(bh) {
    try {
      ...
      //appendnew_next_sd_7h1N1DtKW2Z1nvBf
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_7h1N1DtKW2Z1nvBf');
    }
  }

  sd_VVCZsbA3shYu72Xs(bh) {
    try {
      localStorage.setItem('selectedPage', JSON.stringify('Login'));
      bh = this.sd_pEqAuZJOE57IrHVd(bh);
      //appendnew_next_sd_VVCZsbA3shYu72Xs
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_VVCZsbA3shYu72Xs');
    }
  }

  async sd_pEqAuZJOE57IrHVd(bh) {
    try {
      const { paramObj: qprm, path: path } =
      ...
      await this.__page_injector__
        .get(Router)
        .navigate([this.sdService.formatPathWithParams(path, undefined)]);
      //appendnew_next_sd_pEqAuZJOE57IrHVd
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_pEqAuZJOE57IrHVd');
    }
  }
